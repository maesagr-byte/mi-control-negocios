...
st.markdown('<div class="main-title">🍓 Sistema de Control Profesional - Antojos & Granizadas</div>', unsafe_allowed_html=True)

# 2. Base de Datos en Memoria (Inicialización)
# Si es la primera vez que abre, se carga el menú y stock por defecto
...
with col1:
    st.markdown(f'<div class="metric-box"><h4>💰 Ventas del Día</h4><h2>Q {total_ventas_Q:.2f}</h2></div>', unsafe_allowed_html=True)
with col2:
    st.markdown(f'<div class="metric-box"><h4>📦 Productos en Menú</h4><h2>{len(st.session_state.inventario)} Items</h2></div>', unsafe_allowed_html=True)
with col3:
    color_alerta = "#E74C3C" if len(productos_alerta) > 0 else "#2ECC71"
    st.markdown(f'<div class="metric-box" style="border-left-color:{color_alerta}"><h4>🚨 Alertas de Stock Bajo</h4><h2>{len(productos_alerta)} Productos</h2></div>', unsafe_allowed_html=True)
...
with tab1:
    ...
    with col_izq:
        # Buscador por categoría para agilizar el servicio
        ...
        # Obtener datos actuales del producto elegido
...
with tab2:
    ...
    # Editor interactivo de datos integrado (Data Editor Profesional)
    inventario_editado = st.data_editor(
        ...
        num_rows="dynamic" # Te permite añadir o borrar filas enteras de productos
    )
    
    # Botón para consolidar cambios manuales en la tabla
    ...
    # Sección de avisos críticos de abastecimiento
...
with tab3:
    ...
    if st.session_state.historial_ventas.empty:
        ...
        # Mostrar tabla del historial inverso (la última venta primero)
        st.dataframe(st.session_state.historial_ventas.iloc[::-1], use_container_width=True, hide_index=True)
        
        # Botón de seguridad para exportar la contabilidad a Excel / CSV
